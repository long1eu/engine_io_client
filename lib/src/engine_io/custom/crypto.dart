...
class _CryptoUtils {
  ...
  static const List<int> _decodeTable = const <int>[
  ...
  static String bytesToBase64(List<int> bytes, [bool urlSafe = false, bool addLineSeparator = false]) {
    ...
    final List<int> out = new List<int>(outputLen);

    // Encode 24 bit chunks.
    int j = 0, i = 0, c = 0;
    while (i < chunkLength) {
      final int x = ((bytes[i++] << 16) & 0xFFFFFF) | ((bytes[i++] << 8) & 0xFFFFFF) | bytes[i++];
  ...
  static List<int> base64StringToBytes(String input, [bool ignoreInvalidCharacters = true]) {
    ...
    int extrasLen = 0;
    for (int i = 0; i < len; i++) {
      final int c = _decodeTable[input.codeUnitAt(i)];
      if (c < 0) {
        extrasLen++;
    ...
    for (int i = len - 1; i >= 0; i--) {
      final int currentCodeUnit = input.codeUnitAt(i);
      if (_decodeTable[currentCodeUnit] > 0) break;
      if (currentCodeUnit == PAD) padLength++;
    }
    final int outputLen = (((len - extrasLen) * 6) >> 3) - padLength;
    final List<int> out = new List<int>(outputLen);
...
abstract class _HashBase {
  ...
  _HashBase(this._chunkSizeInWords, this._digestSizeInWords, this._bigEndianWords) : _pendingData = <int>[] {
    _currentChunk = new List<int>(_chunkSizeInWords);
    _h = new List<int>(_digestSizeInWords);
  }
  ...
  void _bytesToChunk(List<int> data, int dataIndex) {
    ...
    for (int wordIndex = 0; wordIndex < _chunkSizeInWords; wordIndex++) {
      final int w3 = _bigEndianWords ? data[dataIndex] : data[dataIndex + 3];
      final int w2 = _bigEndianWords ? data[dataIndex + 1] : data[dataIndex + 2];
      final int w1 = _bigEndianWords ? data[dataIndex + 2] : data[dataIndex + 1];
      final int w0 = _bigEndianWords ? data[dataIndex + 3] : data[dataIndex];
      dataIndex += 4;
      int word = (w3 & 0xff) << 24;
      word |= (w2 & _MASK_8) << 16;
      word |= (w1 & _MASK_8) << 8;
  ...
  List<int> _wordToBytes(int word) {
    final List<int> bytes = new List<int>(_BYTES_PER_WORD);
  ...
  // chunk.
  void _iterate() {
    final int len = _pendingData.length;
    final int chunkSizeInBytes = _chunkSizeInWords * _BYTES_PER_WORD;
  ...
  void _finalizeData() {
    ...
    final int lengthInBits = _lengthInBytes * _BITS_PER_BYTE;
...
class _MD5 extends _HashBase {
  ...
  static const List<int> _k = const <int>[
  ...
  static const List<int> _r = const <int>[
  ...
  void _updateHash(List<int> m) {
    ...
    for (int i = 0; i < 80; i++) {
      if (i < 16) {
        ...
        final int n = _w[i - 3] ^ _w[i - 8] ^ _w[i - 14] ^ _w[i - 16];
        _w[i] = _rotl32(n, 1);
      }
