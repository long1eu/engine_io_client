...
class _WebSocketProtocolTransformer extends StreamTransformerBase<List<int>, dynamic /*List<int>|_WebSocketPing|_WebSocketPong*/ >
  ...
  final List<int> _maskingBytes = new List<int>(4);
  ...
  Stream<dynamic /*List<int>|_WebSocketPing|_WebSocketPong*/ > bind(Stream<List<int>> stream) {
    return new Stream<dynamic>.eventTransformed(stream, (EventSink<dynamic> eventSink) {
      if (_eventSink != null) {
  ...
  void _unmask(int index, int length, Uint8List buffer) {
    ...
    if (length >= BLOCK_SIZE) {
      ...
      if (blockCount > 0) {
        ...
        final Int32x4 blockMask = new Int32x4(mask, mask, mask, mask);
        final Int32x4List blockBuffer = new Int32x4List.view(buffer.buffer, index, blockCount);
...
class _WebSocketPerMessageDeflate {
  ...
  _WebSocketPerMessageDeflate(
      {this.clientMaxWindowBits: WebSocketImpl.DEFAULT_WINDOW_BITS,
      this.serverMaxWindowBits: WebSocketImpl.DEFAULT_WINDOW_BITS,
      this.serverNoContextTakeover: false,
      this.clientNoContextTakeover: false,
      this.serverSide: false});

  void _ensureDecoder() {
    decoder ??= new RawZLibFilter.inflateFilter(windowBits: serverSide ? clientMaxWindowBits : serverMaxWindowBits, raw: true);
  }

  void _ensureEncoder() {
    encoder ??= new RawZLibFilter.deflateFilter(windowBits: serverSide ? serverMaxWindowBits : clientMaxWindowBits, raw: true);
  }
    ...
    if (msg is! Uint8List) {
      for (int i = 0; i < msg.length; i++) {
        if (msg[i] < 0 || 255 < msg[i]) {
          throw new ArgumentError('List element is not a byte value (value ${msg[i]} at index $i)');
        }
      }
...
class _WebSocketOutgoingTransformer extends StreamTransformerBase<dynamic, List<int>> implements EventSink<dynamic> {
  ...
  static Iterable<List<int>> createFrame(int opcode, List<int> data, bool serverSide, bool compressed) {
    ...
    final Uint8List header = new Uint8List(headerSize);
    int index = 0;
    ...
    if (mask) {
      ...
      if (data != null) {
          ...
          if (data is Uint8List) {
            ...
            for (int i = 0; i < data.length; i++) {
              if (data[i] < 0 || 255 < data[i]) {
                throw new ArgumentError('List element is not a byte value (value ${data[i]} at index $i)');
              }
              list[i] = data[i];
            }
          }
        ...
        if (blockCount > 0) {
          ...
          final Int32x4 blockMask = new Int32x4(mask, mask, mask, mask);
          final Int32x4List blockBuffer = new Int32x4List.view(list.buffer, 0, blockCount);
          for (int i = 0; i < blockBuffer.length; i++) {
            blockBuffer[i] ^= blockMask;
          }
        }
    ...
    if (data == null) {
      ...
      return <Uint8List>[header, data];
    }
  }
}

class _WebSocketConsumer implements StreamConsumer<String> {
  final WebSocketImpl webSocket;
  final Socket socket;
  ...
  bool _closed = false;
  final Completer<WebSocket> _closeCompleter = new Completer<WebSocket>();
  ...
  void _ensureController() {
    ...
    _controller = new StreamController<dynamic>(sync: true, onPause: _onPause, onResume: _onResume, onCancel: _onListen);
    final Stream<List<int>> stream = _controller.stream.transform(new _WebSocketOutgoingTransformer(webSocket));
    socket.addStream(stream).then((dynamic _) {
      _done();
  ...
  Future<WebSocket> addStream(Stream<dynamic> stream) {
    if (_closed) {
      ...
      return new Future<WebSocket>.value(webSocket);
    }
    _ensureController();
    _completer = new Completer<WebSocket>();
  ...
  Future<WebSocket> close() {
    ...
    Future<WebSocket> closeSocket() {
      return socket.close().catchError((dynamic _) {}).then<WebSocket>((dynamic _) => webSocket);
    }

    _controller.close();
    return _closeCompleter.future.then<WebSocket>((_) => closeSocket());
  }
...
class WebSocketImpl extends Stream<dynamic> with _ServiceObject implements WebSocket {
  ...
  static Future<WebSocket> connect(String url,
    ...
    final Uint8List nonceData = new Uint8List(16);
    for (int i = 0; i < 16; i++) {
    ...
    return httpClient.openUrl('GET', uri).then((HttpClientRequest request) {
      ...
      if (headers != null) {
        headers.forEach((String field, dynamic value) => request.headers.add(field, value));
      }
      ...
      return response.detachSocket().then<WebSocket>((Socket socket) {
        final WebSocketImpl wsi = new WebSocketImpl._fromSocket(socket, protocol, compression, false, deflate);
  ...
  WebSocketImpl._fromSocket(this._socket, this.protocol, CompressionOptions compression,
      [this._serverSide = false, _WebSocketPerMessageDeflate deflate]) {
    _consumer = new _WebSocketConsumer(this, _socket);
    _sink = new _StreamSinkImpl<dynamic>(_consumer);
    _readyState = WebSocket.OPEN;
    _deflate = deflate;

    final _WebSocketProtocolTransformer transformer = new _WebSocketProtocolTransformer(_serverSide, _deflate);
    _subscription = _socket.transform<dynamic>(transformer).listen((dynamic data) {
    ...
    _subscription.pause();
    _controller = new StreamController<dynamic>(
        sync: true,
  ...
  Future<dynamic> close([int code, String reason]) {
    ...
    if (!_controller.isClosed) {
      ...
      if (!_controller.hasListener && _subscription != null) {
        _controller.stream.drain<dynamic>().catchError((dynamic _) => <int, WebSocketImpl>{});
      }
  ...
  static bool _isReservedStatusCode(int code) {
    return code != null &&
        (code < WebSocketStatus.NORMAL_CLOSURE ||
            ...
            (code > WebSocketStatus.INTERNAL_SERVER_ERROR && code < WebSocketStatus.RESERVED_1015) ||
